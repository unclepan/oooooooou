...
class Utils {
  ...
  stringSplice(str, start, end, pass) {
    ...
    pass = pass === '' ? '=' : pass
    start += pass
    let ps = str.indexOf(start)
    ps = ps > 0 ? ps + start.length : 0
    let pe = str.indexOf(end, ps)
  ...
  checkWhiteList() {
    ...
    const href = con.win.location.href
    let flag = false
    for (let i = 0; i < con.whiteList.length; i++) {
      if (href.includes(con.whiteList[i])) {
        flag = true
        break
      }
    }
  ...
  setCookie(name, value, days, domain) {
    ...
    if (days === undefined || days === null || days === '') {
      ...
      const now = new Date()
      const time = now.getTime() + con.DAY * days
      now.setTime(time)
      con.doc.cookie =
        name +
        '=' +
