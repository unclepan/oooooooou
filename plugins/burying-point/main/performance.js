...
class Per {
  ...
  start(isShow) {
    ...
    this.data.enteriesResouceDataFormat = this._setEnteries(
      this.enteriesResouceData
    )
    if (isShow === 'console') {
      this._show()
    } else if (isShow === 'pushQueueData') {
      BP.pushQueueData('per', {
        ...this.data.timingFormat,
        ...this.data.enteriesResouceDataFormat,
        pre: '性能监控'
      })
    }
  }
  _show() {
    ...
    for (const key in this.data.enteriesResouceDataFormat) {
      console.group(
        key +
          '--- 共加载时间' +
          formatMs(this.data.enteriesResouceDataTiming[key])
      )
  ...
  _setTiming(timing) {
    const data = {
      ...
      'domready时间(用户可操作时间节点)': formatMs(
        timing.domContentLoadedEventEnd - timing.navigationStart
      ),
      'onload时间(总下载时间)': formatMs(
